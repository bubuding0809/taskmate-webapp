...
const useUpdateDueDatetime = () => {
  // const queryClient = useQueryClient();
  const { data: sessionData } = useSession();

  const utils = api.useContext();

  return api.task.updateDueDatetime.useMutation({
    onMutate: async (variables) => {
      const { boardId, panelId, taskId, dueDate } = variables;
...
export default useUpdateDueDatetime;
